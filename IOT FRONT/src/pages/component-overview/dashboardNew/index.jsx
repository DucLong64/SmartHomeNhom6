...
export default function DashboardNew() {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      fetchData();
    }, 5000); // Gọi API mỗi 1 giây

    return () => clearInterval(intervalId); // Dọn dẹp interval khi component unmount
  }, []); // useEffect sẽ chỉ chạy một lần khi component mount

  // Lấy dữ liệu mới nhất từ mảng
  const latestData = data.length > 0 ? data[data.length - 1] : { temperature: 0, humidity: 0, light: 0, air_quality: 0 };
      ...
      <Row gutter={20} className='chart-button'>
        <Col span={16}>
          <Card bordered={false} className="chart-container"> 
            <Chart data={data.slice(-20)} /> {/* Hiển thị 20 bản gần nhất */}
          </Card>
        </Col>
        <Col span={8}>
